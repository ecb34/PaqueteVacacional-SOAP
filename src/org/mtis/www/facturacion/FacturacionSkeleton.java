...
public class FacturacionSkeleton {
	...
	private static void addContent(Document document, String nombre, float precio) throws DocumentException {
		Paragraph content = new Paragraph();
		// We add one empty line
		addEmptyLine(content, 1);
		// Lets write a big header
		content.add(new Paragraph("Paquete contratado", catFont));
		addEmptyLine(content, 2);
		content.add(new Paragraph(nombre + " Precio: " + precio, catFont));
	...
	public org.mtis.www.facturacion.CrearFacturaResponse crearFactura(
		...
		try {
			String pdfName = "../factura_" + crearFactura.getIdReserva() + ".pdf";
			...
			String query = String.format("SELECT nombre,precio FROM paquete WHERE id=%s", crearFactura.getIdReserva());
			db.result = db.statement.executeQuery(query);
			if(db.result.first()){
				addContent(document,db.result.getString(1),db.result.getFloat(2));
